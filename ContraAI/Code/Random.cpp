...
//---------------------------
Bitmap * RANDOM::m_BmpSpritePtr = 0;

RANDOM::RANDOM(DOUBLE2 pos) : ObjectBase(pos), m_AnimationTick(0), m_PointingDir(), m_TimeToDie(30), m_ObjectListPtr(0), m_ShootCounter(0)
...
RANDOM::~RANDOM() { // dont delete the static Bmp
...
void RANDOM::StartToDie() { // simpel maar praktis
							//OutputDebugString("\nAauw, RANDOM Hitted!");
...
void RANDOM::Tick(double deltaTime)
{
	if (m_InvincebleTimer>0)
	...
	if (GAME_ENGINE->IsKeyDown('S')) {
		/*String gs1 = "Score ";
		gs1 += NumberToString(GAMESCORE);
		GAME_ENGINE->MessageBox(gs1);
		GAME_ENGINE->SetColor(200, 200, 200);
		GAME_ENGINE->DrawString(String("hello"), (100, 100), 100, 30);*/
...
void RANDOM::CalculateStateAndFrame() {
...
void RANDOM::CalculateBulletSpawn()
{
...
void RANDOM::SetGlobalVars(ObjectList *objectListPtr, DOUBLE2 LevelMax)
{
...
bool RANDOM::DoCollitions(HitRegion *hitVertPtr, DOUBLE2& posRef, DOUBLE2& velocityRef, HitRegion *levelHitPtr)
{
	...
	double deltaX= ReturnGoodLen(collisionrecth.left, collisionrecth.RANDOM, (*pos).x);

	if( deltaX!=0 ){
	RECT2 checkWidth= (*hitHoriPtr)->GetBounds();
	double width= abs( checkWidth.RANDOM- checkWidth.left );
...
double RANDOM::ReturnGoodLen(double getal1, double getal2, double centerXY)
{
	double delta = 0;
