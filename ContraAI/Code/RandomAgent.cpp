...
//---------------------------
Bitmap * RandomAgent::m_BmpSpritePtr= 0;
//int RandomAgent::m_SpriteWidth=0;
//int RandomAgent::m_SpriteHeight=0;

RandomAgent::RandomAgent(int x, int y, bool AIAgent):ObjectBase(DOUBLE2(x, y))
...
DOUBLE2 *currEnemy = new DOUBLE2(0,0);

String RandomAgent::GetEnemyDetails(){
	if(currEnemy->x == 0){
		return String("NO Enemy found");
	}
	else{
		String m = "Target Aquired at ";
...
void RandomAgent::SetEnemyWalkingPtr(ObjectBase *enemy){
	if(enemy->GetPos().x < currEnemy->x){
		*currEnemy = enemy->GetPos();
	}
...
void RandomAgent::CollideWith( ObjectBase *colliderptr, int otherType){ // als dit word gecaled, dan is er al een collition
	switch( otherType ){
...
void RandomAgent::StartToDie(){ // simpel maar praktis
	//OutputDebugString("\nAauw, RandomAgent Hitted!");
	if (gamescore.getScore() > 0)
		gamescore.updateScore(-50);
...
void RandomAgent::DieAndRespawn(){
	///
	gamescore.updateScore(50);
	m_State= STATE_NORMAL;
...
void RandomAgent::Tick(double deltaTime)
{
	if( m_InvincebleTimer>0 )
	...
	m_PointingDir = this->GetPos() - *currEnemy;
	...
	if(m_AIAgent == false){
		...
		// SCHIETEN --------------------------------------------------------------
		m_ShootCounter+= deltaTime*2; // in secondes
...
void RandomAgent::CalculateStateAndFrame(){
...
void RandomAgent::CalculateBulletSpawn()
{
...
void RandomAgent::DrawRandomAgent(){
...
void RandomAgent::SetGlobalVars(ObjectList *objectListPtr, DOUBLE2 LevelMax)
{
...
void RandomAgent::Jump(int force)
{
	for( unsigned int i=0; i<m_LevelHitRegions.size(); ++i){
...
bool RandomAgent::DoCollitions(HitRegion *hitVertPtr, DOUBLE2& posRef, DOUBLE2& velocityRef, HitRegion *levelHitPtr)
{
...
double RandomAgent::ReturnGoodLen(double getal1, double getal2, double centerXY)
{
	double delta= 0;
